...
func trimVersion(goVersion string) string {
	return strings.TrimPrefix(strings.Fields(goVersion)[2], "go")
}
...
go %s
`, trimVersion(r.GoVersion())), "test.mod")
	})
...
go %s
`, trimVersion(r.GoVersion())), "test2.mod")
	})
...
`, trimVersion(r.GoVersion())), "test3.mod")
	})
...
`, trimVersion(r.GoVersion())), "test4.mod")
	})
...
require github.com/yolo/best/v100 v100.0.0 // thebest
`, trimVersion(r.GoVersion())), "test5.mod")
